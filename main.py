...
async def on_member_join(member):
    ...
    print(invites == test)
    print([x.uses for x in test if x.uses not in [y.uses for y in invites]])
...
async def on_message(message):
    ...
    if message.content.split(" ")[0] in ["+kiss","+handhold", "+lick"] and ("275312272975462411" in message.content or "Fork#1441" in message.content):
...
#         await bot.send_message(message.channel, arg)
    ...
    if message.content.startswith(">os.roleme "):
        ...
        if t in ["Cinnamon bun", "Cinnamon Bun", "cinnamon bun", "cinnamon Bun"]:
            ...
            ##await bot.wait_until_ready()
            print("testtestrtestetsetstsetsset")
            await bot.process_commands(message)
            return
